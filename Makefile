...
slack-up:
	cd examples/slack && docker compose up --build

slack-test:
	cd examples/slack && trap 'docker compose down --remove-orphans' EXIT; docker compose up --build -d --remove-orphans && docker compose exec -T api-a python -m pytest -q && python scripts/smoke_test.py

slack-test-clean:
	cd examples/slack && docker compose down -v --remove-orphans

slack-down:
	cd examples/slack && docker compose down --remove-orphans

test: bitly-test dropbox-test gopuff-test ticketmaster-test fb-news-feed-test tinder-test leetcode-test whatsapp-test rate-limiter-test fb-live-comments-test fb-post-search-test youtube-top-k-test uber-test youtube-test web-crawler-test ad-click-aggregator-test news-aggregator-test yelp-test strava-test online-auction-test price-tracking-test instagram-test robinhood-test google-docs-test distributed-cache-test job-scheduler-test payment-system-test metrics-monitoring-test slack-test
